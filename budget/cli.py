...
from .ledger import (
    LedgerRow,
    last_reconcile_date,
    projected_balance_on as _projected_balance_on,
    display_balance_as_of as _display_balance_as_of,
    list_transactions_since_checkpoint,
    months_between,
    occurrence_on_or_before,
    occurrence_after,
    end_of_month,
    ledger_rows as _ledger_rows,
    next_event as _next_event,
)
...
def projected_balance_on(session, account_id: int, as_of: date) -> float:
    return _projected_balance_on(session, account_id, as_of, IRREG_MODE, IRREG_QUANTILE)


def display_balance_as_of(session, account_id: int, as_of: date) -> float:
    return _display_balance_as_of(session, account_id, as_of, IRREG_MODE, IRREG_QUANTILE)
...
def ledger_rows(
    ...
    old_date = ledger_module.date
    ledger_module.date = date
    try:
        return _ledger_rows(session, plan_start, plan_end, account_ids, IRREG_MODE, IRREG_QUANTILE, date.today())
    finally:
        ledger_module.date = old_date
